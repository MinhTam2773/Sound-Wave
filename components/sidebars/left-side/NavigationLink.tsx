      ...
      {navLinks.map((link) =>
        link.action ? (
          <button
            key={link.name}
            onClick={link.action}
            className="flex items-center gap-[10px] w-full min-h-[45px] rounded-[10px] p-0 pl-[15px] hover:bg-white/10 transition-colors cursor-pointer"
          >
            <img
